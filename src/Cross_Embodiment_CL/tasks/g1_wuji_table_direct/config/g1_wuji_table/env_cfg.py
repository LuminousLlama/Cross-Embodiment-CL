...
class G1WujiTableVirtualForceCfg:
    ...
    Isaac Lab 3.0 Newton publishes contact sensors after its internally handled decimation block.
    The same final contact sample is therefore held across these two estimator updates. This preserves
    the fitted estimator's clock but cannot reconstruct the unavailable intermediate contact sample.
    """
...
class G1WujiTableVirtualForcePresetCfg(PresetCfg):
    """Enable virtual force only for student distillation unless explicitly overridden."""
...
class G1WujiTableEnvCfg(DirectRLEnvCfg):
    ...
    """Virtual Wuji actuator-torque diagnostics; ``distill`` enables them without changing student inputs."""
    ...
    ``U(1-m*s, 1+m*s)``."""
    adr_camera_position_enabled: bool = False
    ...
    adr_camera_rotation_enabled: bool = False
    ...
    adr_camera_focal_enabled: bool = True
    ...
    adr_camera_principal_point_enabled: bool = True
    ...
    adr_depth_scale_enabled: bool = True
    ...
    adr_depth_bias_enabled: bool = True
    ...
    adr_depth_pixel_noise_enabled: bool = True
