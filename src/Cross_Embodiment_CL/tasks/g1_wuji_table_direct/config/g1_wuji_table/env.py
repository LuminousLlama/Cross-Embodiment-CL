...
class G1WujiTableEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: G1WujiTableEnvCfg, render_mode: str | None = None, **kwargs) -> None:
        ...
        if self._adr_camera_active:
            ...
            for name, probability in (
                ("adr_depth_missing_return_prob", self.cfg.adr_depth_missing_return_prob),
    ...
    def _randomize_depth_camera(self, env_ids: torch.Tensor, strength: float) -> None:
        ...
        translation = scaled_uniform(
            (count, 3), self.cfg.adr_camera_position_range, strength, device=self.device
        )
        rotation = scaled_uniform(
            (count, 3), math.radians(self.cfg.adr_camera_rotation_range_deg), strength, device=self.device
        )
        ...
        self._adr_camera_focal_scale[env_ids] = 1.0 + scaled_uniform(
            count, self.cfg.adr_camera_focal_scale, strength, device=self.device
        )
        self._adr_camera_principal_point_offset[env_ids] = scaled_uniform(
            (count, 2), self.cfg.adr_camera_principal_point_offset, strength, device=self.device
        )
        self._adr_depth_scale[env_ids] = 1.0 + scaled_uniform(
            count, self.cfg.adr_depth_scale, strength, device=self.device
        )
        self._adr_depth_bias[env_ids] = scaled_uniform(
            count, self.cfg.adr_depth_bias, strength, device=self.device
        )
    ...
    def _randomize_depth_camera_frame(self, depth: torch.Tensor) -> torch.Tensor:
        ...
        return randomize_depth_measurement(
            ...
            noise_std_at_1m_m=self.cfg.adr_depth_noise_std_at_1m,
            missing_return_prob=self.cfg.adr_depth_missing_return_prob,
            boundary_corruption_prob=self.cfg.adr_depth_boundary_corruption_prob,
