...
"""Headless evaluation of an RSL-RL checkpoint with its saved training action noise.

Based on IsaacLab's ``isaaclab_rl/entrypoints/backends/play_rsl_rl.py``: keeps its CLI and
preset parsing, environment creation, runner construction, and ``runner.load``, but drops JIT/ONNX export,
...
parser = argparse.ArgumentParser(
    description="Evaluate an RSL-RL checkpoint with sampled training-policy actions, headless."
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Directory to save headless PNG frames of the sampled rollout plus a tiled contact sheet. "
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    with startup_screen(args_cli, num_stages=3) as screen:
        ...
        with launch_simulation(env_cfg, args_cli):
            ...
            # Training calls this same policy with stochastic_output=True, sampling its saved action distribution.
    ...
    result = {
        ...
        "policy": "random_actions" if args_cli.random_actions else "training_noise",
